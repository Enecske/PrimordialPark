...
public class GeckoModel<T extends GeckoEntity> extends HierarchicalModel<T> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(PrimordialPark.MODID, "gecko"), "main");
    ...
    public void setupAnim(GeckoEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        animateWalk(GeckoAnimations.GECKO_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        animate(entity.IDLE_ANIMATION_STATE, GeckoAnimations.GECKO_IDLE, ageInTicks, 1f);
    }

    private void applyHeadRotation(float yaw, float pitch) {
        yaw = Math.clamp(yaw, -30f, 30f);
        pitch = Math.clamp(pitch, -25f, 45f);

        head.xRot = yaw * ((float) Math.PI / 180f);
        head.yRot = pitch * ((float) Math.PI / 180f);
    }
